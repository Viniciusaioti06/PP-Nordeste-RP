...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit",async event=>{
    ...
    try{
      ...
      result.querySelector("[data-timeline]").innerHTML=(app.timeline||[]).map(item=>`<div class="status-item"><strong>${escapeHTML(item.status)}</strong><span>${formatDate(item.date)}</span></div>`).join("");
